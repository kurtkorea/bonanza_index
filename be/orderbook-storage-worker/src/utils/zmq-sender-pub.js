...
async function init_zmq_pub() {
  if (!pub) {
    try {
      console.log(`Initializing ZMQ Pub socket on host ${process.env.ZMQ_PUB_HOST}...`);
      ...
      console.log(`✅ ZMQ Pub socket initialized successfully on host ${process.env.ZMQ_PUB_HOST}`);
    } catch (error) {
