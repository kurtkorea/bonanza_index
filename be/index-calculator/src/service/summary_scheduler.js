...
class SummaryScheduler {
  ...
  /**
   * 수동으로 Summary 계산을 실행합니다.
   */
  async executeManual() {
    if (this.isRunning) {
      throw new Error('이전 작업이 아직 실행 중입니다.');
    }

    logger.info({
      symbols: this.symbols
    }, '[SummaryScheduler] 수동 Summary 계산 시작');
    ...
    for (const symbol of this.symbols) {
      try {
        ...
        logger.info({
          ...
          success: result.success,
          count: result.count
        }, '[SummaryScheduler] 수동 Summary 계산 완료');
      } catch (error) {
        logger.error({
          symbol,
          err: String(error),
          stack: error.stack
        }, '[SummaryScheduler] 수동 Summary 계산 중 오류');
        throw error;
      }
    }
  }
