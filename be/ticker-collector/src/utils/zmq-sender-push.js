...
async function init_zmq_push() {
  if (!push) {
    try {
      console.log(`Initializing ZMQ Push socket on host ${process.env.ZMQ_PUSH_HOST}...`);
      ...
      console.log(`✅ ZMQ Push socket initialized successfully on host ${process.env.ZMQ_PUSH_HOST}`);
    } catch (error) {
