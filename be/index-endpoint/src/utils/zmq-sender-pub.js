...
async function init_zmq_pub() {
  if (!pub) {
    try {
        console.log(`Initializing ZMQ Pub socket on host ${process.env.ZMQ_PUB_COMMAND_HOST}...`);
      ...
      // 바인딩 시도
      await pub.bind(process.env.ZMQ_PUB_COMMAND_HOST);
      
      // 큐 생성
      q = new ZmqSendQueuePub(pub);
      
      console.log(`✅ ZMQ Pub socket initialized successfully on host ${process.env.ZMQ_PUB_COMMAND_HOST}`);
    } catch (error) {
...
function getZMQStatus() {
  return {
    ...
    port: process.env.ZMQ_PUB_COMMAND_HOST || 'undefined',
