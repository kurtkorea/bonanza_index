...
csv_row_to_ilp() {
  ...
  # order_tp에서 첫 번째 토큰만 사용 (예: "13 A" -> "13")
  local order_tp_clean="${order_tp%% *}"
  ...
  # marketAt: tran_date + tran_time을 조합한 타임스탬프 (KST -> UTC 변환, 마이크로초 단위)
  local market_at_micros
  market_at_micros=$(ts_field_micros "$tran_date" "$tran_time")

  # collectorAt, dbAt: marketAt과 동일한 값 사용
  local collector_at_micros="${market_at_micros}"
  local db_at_micros="${market_at_micros}"

  local esc_exchange_cd
  esc_exchange_cd=$(esc_tag "$exchange_cd")
  local esc_order_tp
  esc_order_tp=$(esc_tag "$order_tp_clean")
  local esc_tran_date
  esc_tran_date=$(esc_tag "$tran_date")
  local esc_tran_time
  esc_tran_time=$(esc_tag "$tran_time")
  ...
  local price_val
  price_val=$(float_field "$price")
  local size_val
  size_val=$(float_field "$size")
  ...
  local fields
  fields=$(IFS=','; echo "${field_parts[*]}")

  # 3) ns 단위 designated timestamp 제거 (marketAt 컬럼 값만 사용)
  echo "${TABLE_NAME},${tags} ${fields}"
}
...
process_csv_file() {
  ...
  if [[ "${CSV_FILE}" == *.gz ]]; then
    ...
    CSV_INPUT_CMD="gunzip -c ${CSV_FILE}"
  else
    echo "[$(date '+%F %T')] Reading CSV file..."
    CSV_INPUT_CMD="cat ${CSV_FILE}"
  fi

  # ILP 연결 및 데이터 전송
  if [ "$USE_NC" = true ]; then
    # 2) netcat: 파일당 한 번만 연결 + 배치 전송
    local line_count=0
    ...
    # persistent nc 연결
    coproc QDB_CONN { nc -w 5 "${QDB_ILP_HOST}" "${QDB_ILP_PORT}"; }
    if [ -z "${QDB_CONN_PID:-}" ]; then
      echo "[$(date '+%F %T')] ERROR: Failed to start nc"
      exit 1
    fi
    ...
    while IFS= read -r line || [ -n "$line" ]; do
      ...
      # ILP 라인 생성
      local ilp_line=""
      ilp_line=$(csv_row_to_ilp "$line" 2>/dev/null) || true
      if [ -z "$ilp_line" ]; then
        skipped_count=$((skipped_count + 1))
        continue
      fi

      batch_buffer+="${ilp_line}"$'\n'
      line_count=$((line_count + 1))

      # 배치 크기에 도달하면 전송
      if [ $((line_count % BATCH_SIZE)) -eq 0 ]; then
        printf '%s' "$batch_buffer" >&3 || {
          echo "[$(date '+%F %T')] ERROR: Failed to send data batch to QuestDB"
          exec 3>&-
          kill "$QDB_CONN_PID" 2>/dev/null || true
          exit 1
        }
        batch_buffer=""
        echo -ne "\r[$(date '+%F %T')] Processed: ${line_count} lines, Skipped: ${skipped_count}"
      fi
    done < <(eval "$CSV_INPUT_CMD")
    ...
    exec 3>&-
    wait "$QDB_CONN_PID" || {
      echo "[$(date '+%F %T')] ERROR: nc connection closed with error"
      exit 1
    }
